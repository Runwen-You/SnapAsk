...
SnapAsk 0.2.0 是面向 Windows 10/11 的截图问答工具。它把 Snipaste 式截图、非破坏性标注、贴图和流式 AI 回答放在同一内存会话中，并以一次规范化快照渲染作为保存、复制、贴图和发送的唯一像素来源。
...
- 截图工具条、编辑器、回答卡、设置和服务弹窗共用 Glassmorphism 设计系统；Windows 11 使用官方 DWM 背景材质，Windows 10、RDP、高对比或不可用环境自动采用高可读降级样式。
